...
(function () {
  ...
  function renderAllSkillsPage() {
    ...
    for (var ti = 0; ti < keys.length; ti++) { var catKey = keys[ti]; var cat = CATEGORIES[catKey]; for (var mi = 0; mi < cat.modules.length; mi++) { var m = cat.modules[mi]; num++; var lc = (m.license || 'Free').replace('+', 'plus').replace(' ', '-').toLowerCase(); rows += '<tr><td>' + num + '</td><td><a href="#" class="all-skills-cat-link" data-page="' + catKey + '"><span class="badge-track">' + escapeHtml(cat.title) + '</span></a></td><td>' + escapeHtml(m.title) + '</td><td>' + escapeHtml(m.skill) + ' <a href="https://github.com/skills-kr/' + m.repo + '" target="_blank" rel="noopener" class="badge badge-ready skill-lab-link">실습</a></td><td><span class="badge badge-license badge-license-' + lc + '">' + escapeHtml(m.license || 'Free') + '</span></td><td><span class="badge badge-time">' + m.time + '분</span></td><td><a href="#" class="badge badge-dash all-skills-link" data-session-id="' + m.sessionId + '">이동</a></td></tr>'; } }
