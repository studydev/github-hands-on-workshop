...
(function () {
  ...
  function renderSessionInfo(s) { var st = s.startDate ? new Date(s.startDate).toLocaleString('ko-KR') : '-', en = s.endDate ? new Date(s.endDate).toLocaleString('ko-KR') : '-'; sessionInfoEl.innerHTML = '<strong>' + escapeHtml(s.name) + '</strong> &nbsp;|&nbsp; 트랙: ' + escapeHtml(s.track) + ' &nbsp;|&nbsp; 기간: ' + st + ' ~ ' + en; }
  ...
  function renderAllSkillsPage() {
    var rows = '', num = 0, keys = Object.keys(TRACKS);
    for (var ti = 0; ti < keys.length; ti++) { var track = TRACKS[keys[ti]]; for (var mi = 0; mi < track.modules.length; mi++) { var m = track.modules[mi]; num++; rows += '<tr><td>' + num + '</td><td><span class="badge-track">' + escapeHtml(track.title) + '</span></td><td>' + escapeHtml(m.skill) + ' <a href="https://github.com/skills-kr/' + m.repo + '" target="_blank" rel="noopener" class="badge badge-ready skill-lab-link">실습</a></td><td>' + escapeHtml(m.title) + '</td><td><a href="#" class="badge badge-dash all-skills-link" data-session-id="' + m.sessionId + '">이동</a></td></tr>'; } }
    pageAllSkills.innerHTML = '<div class="all-skills-page-inner"><p class="all-skills-desc">모든 트랙의 GitHub Skills 실습을 한눈에 확인하세요.</p><div class="curriculum-table-wrapper"><table class="curriculum-table all-skills-table"><thead><tr><th>#</th><th>트랙</th><th>Skill</th><th>모듈</th><th>세션 대시보드</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
